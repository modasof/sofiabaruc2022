 ...
 ********************************************************/
    public static function ReporteCombustiblesporfecha($FechaStart, $FechaEnd)
    {
        try {
            ...
            $select  = $db->query("SELECT * FROM reporte_combustibles WHERE reporte_publicado='1'and fecha_reporte >='" . $FechaStart . "' and fecha_reporte <='" . $FechaEnd . "' order by fecha_reporte DESC");
            $camposs = $select->fetchAll();
            $campos  = new Reportes('', $camposs);
            return $campos;
        } catch (PDOException $e) {
 ...
 ********************************************************/
    public static function ReporteEstacionesporfechameseq($FechaStart, $FechaEnd)
    {
        try {
            ...
            //echo($sql);
            $select  = $db->query("SELECT DISTINCT(punto_despacho),SUM(cantidad) as totalestacion,SUM(cantidad*valor_m3) as totalestacionvalor FROM reporte_combustibles WHERE reporte_publicado='1'  and fecha_reporte >='" . $FechaStart . "' and fecha_reporte <='" . $FechaEnd . "' GROUP by punto_despacho ORDER BY totalestacion DESC");
            $camposs = $select->fetchAll();
            $campos  = new Reportes('', $camposs);
            return $campos;
        } catch (PDOException $e) {
 ...
 ** FUNCION PARA MOSTRAR TODOS LOS CAMPOS POR RANGO DE FECHA      **
 ********************************************************/
    public static function ReporteporPropietario($FechaStart, $FechaEnd)
    {
        try {
            $db = Db::getConnect();

            //echo($sql);
            $select  = $db->query("SELECT DISTINCT(A.propietario) AS propietario FROM equipos as A, reporte_combustibles AS B WHERE A.id_equipo=B.equipo_id_equipo and A.estado_equipo='1' and B.fecha_reporte >='" . $FechaStart . "' and B.fecha_reporte <='" . $FechaEnd . "'");
            $camposs = $select->fetchAll();
            $campos  = new Reportes('', $camposs);
            return $campos;
        } catch (PDOException $e) {
    ...
    public static function ReporteconsumoporPropietario($FechaStart, $FechaEnd, $id)
    {
        try {
            $db = Db::getConnect();

            $select = $db->query("SELECT IFNULL(sum(A.cantidad),0) as total FROM reporte_combustibles as A, equipos as B WHERE A.equipo_id_equipo=B.id_equipo and B.propietario='" . $id . "' and A.fecha_reporte >='" . $FechaStart . "' and A.fecha_reporte <='" . $FechaEnd . "'");
            //echo($sql);
            $camposs = $select->fetchAll();
            $campos  = new Reportes('', $camposs);
            $marcas  = $campos->getCampos();
    ...
    public static function ReportevalorporPropietario($FechaStart, $FechaEnd, $id)
    {
        try {
            $db = Db::getConnect();

            $select  = $db->query("SELECT  IFNULL(sum(A.cantidad*A.valor_m3),0) as valor FROM reporte_combustibles as A, equipos as B WHERE A.equipo_id_equipo=B.id_equipo and B.propietario='" . $id . "' and A.fecha_reporte >='" . $FechaStart . "' and A.fecha_reporte <='" . $FechaEnd . "'");
            $camposs = $select->fetchAll();
            $campos  = new Reportes('', $camposs);
            $marcas  = $campos->getCampos();
            foreach ($marcas as $marca) {
 ...
 ***************************************************************/
    public static function guardarhoras($campos)
    {
        try {
            ...
            $insert = $db->prepare('INSERT INTO reporte_horas VALUES (NULL,:fecha_reporte,:equipo_id_equipo,:cliente_id_cliente,:despachado_por,:punto_despacho,:recibido_por,:valor_m3, :cantidad,:indicador,:hora_inactiva,:creado_por, :estado_reporte, :reporte_publicado,:marca_temporal, :observaciones)');

            $totalkm = round($cantidad - $indicador, 2);
            ...
            $insert->bindValue('cantidad', utf8_decode($cantidad));
            $insert->bindValue('indicador', utf8_decode($indicador));
            $insert->bindValue('hora_inactiva', utf8_decode($totalkm));
            $insert->bindValue('creado_por', utf8_decode($creado_por));
            $insert->bindValue('estado_reporte', utf8_decode($estado_reporte));
            $insert->bindValue('reporte_publicado', utf8_decode($reporte_publicado));
            $insert->bindValue('marca_temporal', utf8_decode($marca_temporal));
 ...
 ***************************************************************/
    public static function eliminarhorasPor($id)
    {
        try {
            $db     = Db::getConnect();
            $select = $db->query("DELETE FROM reporte_horas WHERE id='" . $id . "'");
            if ($select) {
                return true;
            } else {return false;}
        } catch (PDOException $e) {
...
********************************************************/
public static function validarkmpor($kminicial,$kmfinal,$equipo_id_equipo){
	try {
		$db=Db::getConnect();

		$select=$db->query("SELECT COUNT(id) AS total FROM reporte_horas WHERE indicador='".$kminicial."' and cantidad='".$kmfinal."' and equipo_id_equipo='".$equipo_id_equipo."'");
    	$camposs=$select->fetchAll();
    	$campos = new Reportes('',$camposs);
    	$marcas = $campos->getCampos();
		foreach($marcas as $marca){
