...
public static function cotizaciones(){
	try {
		$db=Db::getConnect();
		$sql="SELECT DISTINCT(proveedor_id_proveedor) FROM cotizaciones_item as A, proveedores as B WHERE estado_cotizacion='1' and A.proveedor_id_proveedor=B.id_proveedor order by nombre_proveedor ASC";
		//echo($sql);
		$select=$db->query($sql);
    	$camposs=$select->fetchAll();
    	$campos = new Requisiciones('',$camposs);
